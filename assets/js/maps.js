...
(function ( $ ) {
  ...
  //$("#location_search").geocomplete(geooptions);
    ...
    //function mainMap() {
      ...
      // Geo location button
        $(".geoLocation,.main-search-input-item.location a,.location:not(.add-listing-section) a,.form-field-_address-container a").on("click", function (e) {
          e.preventDefault();
          geolocate();
      });
  ...
  if(listeo_core.country){
      ...
      if(listeo_core.submitCenterPoint) {
        ...
        if($('#_geolocation_long').val() && $('#_geolocation_lat').val()) {
         ...
         window.submit_map = new google.maps.Map(document.getElementById('submit_map'), {
          zoom: 10,
          center:submitcenter,
          scrollwheel: false,
          zoomControl: true,
