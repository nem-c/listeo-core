...
class Listeo_Core_Post_Types {
	...
	public function __construct() {
		...
		add_action( 'save_post_listing', array( $this,'save_availibilty_calendar'), 1, 3 );
...
/*				if ( $post->post_status !== 'trash' ) {
	...
	public function set_expiry( $post ) {
		...
		if ( get_post_meta( $post->ID, '_listing_expires', true ) ) {
			$expires =  get_post_meta( $post->ID, '_listing_expires', true );
			...
			if ( $expires && $expires < current_time( 'timestamp' ) ) {
				...
				update_post_meta( $post->ID, '_listing_expires', strtotime($expires) );
			}
