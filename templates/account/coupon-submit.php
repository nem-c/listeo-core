...
<?php
		...
		<div class="add-listing-section row">
			...
			<div class="col-md-12">
				<p class="form-field coupon_amount_field ">
					...
					<textarea style="min-height: 100px;" id="woocommerce-coupon-description" name="excerpt" cols="5" rows="1" placeholder="Description (optional)"><?php if(isset($coupon_id)) echo esc_html($data->coupon_data->post_excerpt); ?></textarea>
				</p>
			</div>
			<div class="col-md-12">
					...
					<div 
					data-photo="<?php echo $coupon_bg_url; ?>" 
					data-name="<?php esc_html_e('Coupon Widget Background', 'listeo_core'); ?>" 
					data-size="<?php echo filesize( get_attached_file( $coupon_bg ) ); ?>" 
					class="edit-coupon-photo">
			...
			<div id="general_coupon_data" class="panel woocommerce_options_panel" style="display: block;">
				...
				<div class="col-md-4">
					<p class="form-field coupon_amount_field ">
						...
						<input type="text" class="short wc_input_decimal" style="" required name="coupon_amount" id="coupon_amount" value="<?php if(isset($coupon_id)) echo esc_html(get_post_meta($coupon_id,'coupon_amount',true)); ?>" placeholder="0"> 
					</p>
				</div>
		...
		<div id="usage_restriction_coupon_data" class="panel woocommerce_options_panel" style="display: block;">
				...
				<div class="col-md-6">
					<p class="form-field minimum_amount_field ">
						...
						<input type="text" class="short wc_input_price" style="" name="minimum_amount" id="minimum_amount" value="<?php if(isset($coupon_id)) echo esc_html(get_post_meta($coupon_id,'minimum_amount',true)); ?>" placeholder="No minimum"> 
					</p>
				</div>
				<div class="col-md-6">
					<p class="form-field maximum_amount_field ">
						<label for="maximum_amount"><?php esc_html_e('Maximum spend','listeo_core'); ?></label>
						<input type="text" class="short wc_input_price" style="" name="maximum_amount" id="maximum_amount" value="<?php if(isset($coupon_id)) echo esc_html(get_post_meta($coupon_id,'maximum_amount',true)); ?>" placeholder="No maximum"> 
					</p>
				</div>
					<div class="col-md-6">
				<p class="form-field">
					...
					<select class="select2-single" multiple="" style="width: 50%;" name="listing_ids[]" 
					data-placeholder="<?php esc_html_e('Search for a listing','listeo_core'); ?>" data-action="" tabindex="-1" aria-hidden="true">
						...
						foreach( $posts as $post ) : setup_postdata($post); ?>
						    <option <?php if(in_array($post->ID,$listing_ids_array)) { echo "selected"; } ?> value="<?php echo $post->ID; ?>"><?php the_title(); ?></option>
						<?php endforeach; ?>
						</select>
				...
				<div class="col-md-6">
					<p class="form-field individual_use_field ">
						...
						<div class="switch_box box_1"><input <?php if(isset( $data->coupon_edit ) && get_post_meta($coupon_id,'individual_use',true) == "yes") { echo 'checked="checked"';} ?>type="checkbox" class="input-checkbox switch_1"  style="" name="individual_use" id="individual_use" value="yes"> </div>
			...
			<div id="usage_limit_coupon_data" class="panel woocommerce_options_panel" style="display: block;">
					<div class="col-md-6">
						<p class="form-field usage_limit_field ">
							...
							<input type="number" class="short" style="" name="usage_limit" id="usage_limit" value="<?php if(isset($coupon_id)) echo esc_html(get_post_meta($coupon_id,'usage_limit',true)); ?>" placeholder="Unlimited usage" step="1" min="0"> 
						</p>
					</div>
	...
	<p>
		...
		<button type="submit" value="Submit Coupon" name="submit_coupon"  class="button margin-top-20"><i class="fa fa-arrow-circle-right"></i> 
			<?php if ( isset( $data->coupon_edit ) && $data->coupon_edit ) { esc_html_e('Update Coupon','listeo_core'); } else { esc_html_e('Submit Coupon','listeo_core'); }?>
		</button>
